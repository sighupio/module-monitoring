...
CHART_VERSION=$(helm search repo grafana/mimir-distributed -o json | yq '.[0].version')
echo "Found chart version $CHART_VERSION"

rm -rf /tmp/mimir-distributed
helm pull grafana/mimir-distributed --version "${CHART_VERSION}" --untar --untardir /tmp
echo "Downloaded chart with version $CHART_VERSION in /tmp/mimir-distributed"
