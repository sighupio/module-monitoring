...
2. We add two additional API services via the apiService-EnhancedHPAMetrics.yaml file that are not included in upstreamed manifests. See: https://github.com/sighupio/module-monitoring/issues/191
3. We add the two additional API services to the ClusterRole via a patch, so the adapter can serve them.
